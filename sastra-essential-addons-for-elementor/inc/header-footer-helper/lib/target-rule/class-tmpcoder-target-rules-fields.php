<?php
/**
 * Advanced Headers Bar Post Meta Box
 *
 */
 ...
 * Meta Boxes setup
 */
class TMPCODER_Target_Rules_Fields {
	...
	public function __construct() {
		define( 'TMPCODER_TARGET_RULE_URI', plugins_url( '/', __FILE__ ) );
		define( 'TMPCODER_TARGET_RULE_VER', '1.0.0' );
		define( 'TMPCODER_TARGET_RULE_DIR', plugin_dir_path( __FILE__ ) );

		add_action( 'admin_action_edit', array( $this, 'initialize_options' ) );
		add_action( 'wp_ajax_tmpcoder_get_posts_by_query', array( $this, 'tmpcoder_get_posts_by_query' ) );

        // conditional field hide on elementor pages
	...
	public static function get_location_selections() {
        ...
        unset($custom_post_type['theme-advanced-hook']);
        unset($custom_post_type['tmpcoder_mega_menu']);

		$post_types = apply_filters( 'tmpcoder_location_rule_post_types', array_merge( $post_types, $custom_post_type ) );

		$special_pages = array(
			'special-404'    => __( '404 Page (Pro)', 'sastra-essential-addons-for-elementor' ),
			'special-search' => __( 'Search Page (Pro)', 'sastra-essential-addons-for-elementor' ),
			'special-blog'   => __( 'Blog / Posts Page (Pro)', 'sastra-essential-addons-for-elementor' ),
			'special-front'  => __( 'Front Page (Pro)', 'sastra-essential-addons-for-elementor' ),
			'special-date'   => __( 'Date Archive (Pro)', 'sastra-essential-addons-for-elementor' ),
			'special-author' => __( 'Author Archive (Pro)', 'sastra-essential-addons-for-elementor' ),
		);

		if ( class_exists( 'WooCommerce' ) ) {
			$special_pages['special-woo-shop'] = __( 'WooCommerce Shop Page (Pro)', 'sastra-essential-addons-for-elementor' );
		}

		$selection_options = array(
			'basic'         => array(
				'label' => __( 'Basic', 'sastra-essential-addons-for-elementor' ),
				'value' => array(
					'basic-global'    => __( 'Entire Website', 'sastra-essential-addons-for-elementor' ),
					'basic-singulars' => __( 'All Singulars (Pro)', 'sastra-essential-addons-for-elementor' ),
					'basic-archives'  => __( 'All Archives (Pro)', 'sastra-essential-addons-for-elementor' ),
				),
			),

			'special-pages' => array(
				'label' => __( 'Special Pages', 'sastra-essential-addons-for-elementor' ),
				'value' => $special_pages,
			),
		);
		...
		$selection_options['specific-target'] = array(
			'label' => __( 'Specific Target', 'sastra-essential-addons-for-elementor' ),
			'value' => array(
				'specifics' => __( 'Specific Pages / Posts / Taxonomies, etc. (Pro)', 'sastra-essential-addons-for-elementor' ),
			),
		);
		 ...
		 * @since 1.5.0
		 */
		return apply_filters( 'tmpcoder_display_on_list', $selection_options );
	}
	...
	public static function get_user_selections() {
		$selection_options = array(
			'basic'    => array(
				'label' => __( 'Basic', 'sastra-essential-addons-for-elementor' ),
				'value' => array(
					'all'        => __( 'All', 'sastra-essential-addons-for-elementor' ),
					'logged-in'  => __( 'Logged In', 'sastra-essential-addons-for-elementor' ),
					'logged-out' => __( 'Logged Out', 'sastra-essential-addons-for-elementor' ),
				),
			),

			'advanced' => array(
				'label' => __( 'Advanced', 'sastra-essential-addons-for-elementor' ),
				'value' => array(),
			),
		);
		 ...
		 * @since 1.5.0
		 */
		return apply_filters( 'tmpcoder_user_roles_list', $selection_options );
	}
	 ...
	 * @since  1.0.0
	 */
	function tmpcoder_get_posts_by_query() {

		if ( !isset($_POST['nonce']) || !wp_verify_nonce( sanitize_text_field(wp_unslash($_POST['nonce'])), 'tmpcoder-get-posts-by-query')  || !current_user_can( 'manage_options' ) ) {
		...
		unset( $post_types['theme-advanced-hook'] ); //Exclude Site Builder templates.
		...
		foreach ( $post_types as $key => $post_type ) {
			...
			if ( $layout_type == 'type_single_product' && $post_type != 'product' ){
				continue;
			}
	 ...
	 * This is only run from tmpcoder_get_posts_by_query()
	 *
	...
	public function admin_styles() {
		wp_enqueue_script( 'tmpcoder-select2', TMPCODER_TARGET_RULE_URI . 'select2'. tmpcoder_script_suffix() .'.js', array( 'jquery' ), TMPCODER_TARGET_RULE_VER, true );
		...
		if ( '' !== $wp_lang ) {
			...
			if ( isset( $select2_lang[ $wp_lang ] ) && file_exists( TMPCODER_TARGET_RULE_DIR . 'i18n/' . $select2_lang[ $wp_lang ] . '.js' ) ) {
				$ast_lang = $select2_lang[ $wp_lang ];
				wp_enqueue_script(
					'tmpcoder-select2-lang',
					TMPCODER_TARGET_RULE_URI . 'i18n/' . $select2_lang[ $wp_lang ] . '.js',
					array(
						'jquery',
						'tmpcoder-select2',
					),
					TMPCODER_TARGET_RULE_VER,
					true
				);
			}
		}
		...
		wp_register_script(
			'tmpcoder-target-rule',
			TMPCODER_TARGET_RULE_URI . 'target-rule'.tmpcoder_script_suffix().'.js',
			array(
				'jquery',
				'tmpcoder-select2',
			),
		...
		wp_enqueue_script( 'tmpcoder-target-rule' );

		wp_register_script(
			'tmpcoder-user-role',
			TMPCODER_TARGET_RULE_URI . 'user-role'.tmpcoder_script_suffix().'.js',
			array(
				'jquery',
			),
			TMPCODER_TARGET_RULE_VER,
		...
		wp_enqueue_script( 'tmpcoder-user-role' );

		wp_register_style( 'tmpcoder-select2', TMPCODER_TARGET_RULE_URI . 'select2'. tmpcoder_script_suffix().'.css', '', TMPCODER_TARGET_RULE_VER );
		wp_enqueue_style( 'tmpcoder-select2' );
		wp_register_style( 'tmpcoder-target-rule', TMPCODER_TARGET_RULE_URI . 'target-rule'.tmpcoder_script_suffix().'.css', '', TMPCODER_TARGET_RULE_VER );
		wp_enqueue_style( 'tmpcoder-target-rule' );
		...
		$localize_vars = array(
			'ast_lang'      => $ast_lang,
			'please_enter'  => __( 'Please enter', 'sastra-essential-addons-for-elementor' ),
			'please_delete' => __( 'Please delete', 'sastra-essential-addons-for-elementor' ),
			'more_char'     => __( 'or more characters', 'sastra-essential-addons-for-elementor' ),
			'character'     => __( 'character', 'sastra-essential-addons-for-elementor' ),
			'loading'       => __( 'Loading more results…', 'sastra-essential-addons-for-elementor' ),
			'only_select'   => __( 'You can only select', 'sastra-essential-addons-for-elementor' ),
			'item'          => __( 'item', 'sastra-essential-addons-for-elementor' ),
			'char_s'        => __( 's', 'sastra-essential-addons-for-elementor' ),
			'no_result'     => __( 'No results found', 'sastra-essential-addons-for-elementor' ),
			'searching'     => __( 'Searching…', 'sastra-essential-addons-for-elementor' ),
			'not_loader'    => __( 'The results could not be loaded.', 'sastra-essential-addons-for-elementor' ),
			'search'        => __( 'Search pages / post / categories', 'sastra-essential-addons-for-elementor' ),
			'ajax_nonce'    => wp_create_nonce( 'tmpcoder-get-posts-by-query' ),
		);
		wp_localize_script( 'tmpcoder-select2', 'astRules', $localize_vars );
	}
	...
	public static function target_rule_settings_field( $name, $settings, $value='' ) {
		...
		$add_rule_label = isset( $settings['add_rule_label'] ) ? $settings['add_rule_label'] : __( 'Add Rule', 'sastra-essential-addons-for-elementor' );
		$saved_values   = $value;
		$output         = '';
		...
		/* WP Template Format */
		$output .= apply_filters('tmpcoder_target_rule_settings_field', $output, $rule_type,$input_name, $selection_options);

		/* Wrapper Start */
		$output .= '<div class="tmpcoder-target-rule-wrapper tmpcoder-target-rule-' . $rule_type . '-on-wrap" data-type="' . $rule_type . '">';
		// $output .= '<input type="hidden" class="form-control tmpcoder-input tmpcoder-target_rule-input" name="' . esc_attr( $input_name ) . '" value=' . $value . ' />';
		$output .= '<div class="tmpcoder-target-rule-selector-wrapper tmpcoder-target-rule-' . $rule_type . '-on">';
		...
		if (tmpcoder_is_availble()) {
			echo wp_kses(str_replace(' (Pro)', '', $output), array(
                'script' => array(
                    'type'=> array(),
                    'id' => array(),
                ),
	...
	public static function get_post_target_rule_options( $post_type, $taxonomy ) {
		...
		$all_posts                          = sprintf( __( 'All %s (Pro)', 'sastra-essential-addons-for-elementor' ), $post_label );
		$post_option[ $post_name . '|all' ] = $all_posts;

		if ( 'pages' != $post_key ) {
			/* translators: %s post label */
			$all_archive                                = sprintf( __( 'All %s Archive (Pro)', 'sastra-essential-addons-for-elementor' ), $post_label );
		...
		if ( in_array( $post_type->name, $taxonomy->object_type ) ) {
			...
			$tax_archive = sprintf( __( 'All %s Archive (Pro)', 'sastra-essential-addons-for-elementor' ), $tax_label );
	...
	public static function generate_target_rule_selector( $type, $selection_options, $input_name, $saved_values, $add_rule_label ) {
		...
		foreach ( $saved_values['rule'] as $index => $data ) {
			...
			$output .= '<div class="tmpcoder-target-rule-condition tmpcoder-target-rule-' . $index . '" data-rule="' . $index . '" >';
			/* Condition Selection */
			...
			$output .= '<select name="' . esc_attr( $input_name ) . '[rule][' . $index . ']" class="target_rule-condition form-control tmpcoder-input">';
			$output .= '<option value="">' . __( 'Select', 'sastra-essential-addons-for-elementor' ) . '</option>';
			...
			$output .= '<select name="' . esc_attr( $input_name ) . '[specific][]" class="target-rule-select2 target_rule-specific-page form-control tmpcoder-input " multiple="multiple">';
		...
		if (tmpcoder_is_availble()) {
			$output .= '<a href="#" class="button tmpcoder-add-conditions" data-rule-id="' . absint( $index ) . '" data-rule-type="' . $type . '">' . $add_rule_label . '</a>';
		}
		...
		if ( 'display' == $type ) {
			...
			$output .= '<a href="#" class="button">' . __( 'Add Exclusion Rule', 'sastra-essential-addons-for-elementor' ) . '</a>';
			$output .= '</div>';
		}
	...
	public function get_current_layout( $layout_id, $option ) {
		...
		$current_layout = apply_filters( 'tmpcoder_target_page_settings', $current_layout, $layout_id );
	...
	public static function target_user_role_settings_field( $name, $settings, $value ) {
		...
		/* WP Template Format */
		$output .= apply_filters('tmpcoder_target_user_role_settings_field', $output, $input_name, $selection_options);
		...
		$output         .= '<div class="tmpcoder-user-role-wrapper tmpcoder-user-role-display-on-wrap" data-type="display">';
			$output     .= '<div class="tmpcoder-user-role-selector-wrapper tmpcoder-user-role-display-on">';
				$output .= '<div class="user_role-builder-wrap">';
		foreach ( $saved_values as $index => $data ) {
			$output     .= '<div class="tmpcoder-user-role-condition tmpcoder-user-role-' . $index . '" data-rule="' . $index . '" >';
				$output .= '<span class="user_role-condition-delete dashicons dashicons-dismiss"></span>';
				/* Condition Selection */
				$output     .= '<div class="user_role-condition-wrap" >';
					$output .= '<select name="' . esc_attr( $input_name ) . '[' . $index . ']" class="user_role-condition form-control tmpcoder-input">';
					$output .= '<option value="">' . __( 'Select', 'sastra-essential-addons-for-elementor' ) . '</option>';
			...
			foreach ( $selection_options as $group => $group_data ) {
				...
				$output .= '</optgroup>';
					...
					$output .= '</select>';
				$output     .= '</div>';
					$output .= '</div> <!-- tmpcoder-user-role-condition -->';
		}
				$output .= '</div>';
				/* Add new rule */
	...
	public function get_posts_by_conditions( $post_type, $option ) {
		...
		if ( false === $meta_header ) {
            ...
            WHERE pm.meta_key = %s
            AND p.post_type = %s
            AND p.post_status = 'publish'", $location, $post_type);
			...
			switch ( $current_page_type ) {
				...
				case 'is_search':
					if ( $current_post_type != 'page' && $current_post_type != "" ){
						...
						$meta_args .= " OR ( ";
						$meta_args .= " pm.meta_value LIKE '%\"special-search\"%' ";

						$meta_args .= $wpdb->prepare(" AND pm.meta_value LIKE %s  ", "%\"{$current_post_type}|all\"%");
				...
				case 'is_author':
					...
					$meta_args .= $wpdb->prepare(" OR pm.meta_value LIKE %s ","%\"{$current_post_type}|all|archive\"%");
				...
				case 'is_front_page':
					...
					$meta_args .= $wpdb->prepare(" OR pm.meta_value LIKE %s ","%\"{$current_post_type}|all\"%");
                    $meta_args .= $wpdb->prepare(" OR pm.meta_value LIKE %s ","%\"post-{$current_id}\"%");
				...
				case 'is_singular':
					...
					foreach ( $terms as $key => $term ) {
						$meta_args .= $wpdb->prepare(" OR pm.meta_value LIKE %s ", "%\"tax-".$term->term_id."-single-".$term->taxonomy."\"%");
					}
				...
				function() use ( $already_set_rule ) {
					...
					$notice = sprintf(
                        /* translators: %s: post title. */
                         __( 'The same display setting is already exist in %s post/s.', 'sastra-essential-addons-for-elementor' ), $rule_set_titles );
 ...
 */
TMPCODER_Target_Rules_Fields::get_instance();
