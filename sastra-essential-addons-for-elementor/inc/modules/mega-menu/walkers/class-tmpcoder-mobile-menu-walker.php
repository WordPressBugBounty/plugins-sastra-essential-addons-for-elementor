...
class TMPCODER_Mobile_Menu_Walker extends \Walker_Nav_Menu {
	...
	public function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		// Default class.
		$classes = array( 'sub-menu', 'tmpcoder-mobile-sub-menu' );
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( $settings ) {
			if ( 'true' === $settings['tmpcoder_mm_enable'] ) {
				...
				if ( tmpcoder_is_availble() && 'ajax' === $settings['tmpcoder_mm_render'] && 'mega' === $settings['tmpcoder_mm_mobile_content'] ) {
					$classes[] = 'tmpcoder-mega-menu-ajax';
				}
			}
		}
		...
		$item_a_class = 'tmpcoder-mobile-menu-item';
		...
		$output .= $indent .'<li'. $id . $item_li_class . $id_attr .'>';
			...
			$item_output = $args->before;
				...
				// Sub Menu Classes
				if ( $depth > 0 ) {
					$item_a_class = 'tmpcoder-mobile-sub-menu-item';
				...
				$item_output .= '<a'. $attributes .'>';
					...
					$item_output .= in_array( 'menu-item-has-children', $item->classes ) || $this->has_mega_menu($item->ID) ? '<i class="tmpcoder-mobile-sub-icon"></i>' : '';
				$item_output .= '</a>';
	...
	public function has_mega_menu( $item_id ) {
		...
		$mega_content = tmpcoder_is_availble() && isset($settings['tmpcoder_mm_mobile_content']) ? $settings['tmpcoder_mm_mobile_content'] : 'mega';

		return isset($settings['tmpcoder_mm_enable']) && 'true' === $settings['tmpcoder_mm_enable'] && 'mega' === $mega_content ? true : false;
	}
