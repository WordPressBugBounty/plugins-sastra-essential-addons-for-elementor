...
var translationState={isTranslating:!1,totalElements:0,translatedElements:0,failedElements:0,currentElement:null,fromLang:'',toLang:'',isCancelled:!1,currentRequests:[],errorMessages:[]};var languages={'en':'English','es':'Spanish (Español)','fr':'French (Français)','de':'German (Deutsch)','it':'Italian (Italiano)','pt':'Portuguese (Português)','ru':'Russian (Русский)','ja':'Japanese (日本語)','ko':'Korean (한국어)','zh':'Chinese (中文)','ar':'Arabic (العربية)','hi':'Hindi (हिन्दी)','nl':'Dutch (Nederlands)','pl':'Polish (Polski)','tr':'Turkish (Türkçe)','uk':'Ukrainian (Українська)','cs':'Czech (Čeština)','sv':'Swedish (Svenska)','no':'Norwegian (Norsk)','da':'Danish (Dansk)','fi':'Finnish (Suomi)'};function escapeHtml(text){if(typeof text!=='string'){text=String(text)}
...
var $button=$(this);$button.prop('disabled',!0).text('🔍 Verifying...');$.post(SpexoAiPageTranslator.ajax_url,{action:'spexo_ai_text_check_limits',nonce:SpexoAiPageTranslator.generate_nonce},function(response){$button.prop('disabled',!1).text('Start Translation');if(!response.success||!response.data.api_key_valid){var errorMessage='API key verification failed. Please check your API key in settings.';if(response.data&&response.data.error_message){errorMessage=response.data.error_message}
