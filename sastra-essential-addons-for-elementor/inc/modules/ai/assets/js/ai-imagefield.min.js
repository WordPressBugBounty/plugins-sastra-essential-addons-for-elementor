...
var imageModels=(SpexoAiImageField.image_models&&SpexoAiImageField.image_models.length)?SpexoAiImageField.image_models:[{value:'dall-e-3',label:'DALL·E 3'},{value:'dall-e-2',label:'DALL·E 2'},{value:'gpt-image-1',label:'GPT Image 1'}];var modelControls=SpexoAiImageField.model_controls||{};imageModels.forEach(function(m){$('<option/>').attr('value',m.value).text(m.label).appendTo($modelSelect)});if(is_pro&&SpexoAiImageField.image_model){$modelSelect.val(SpexoAiImageField.image_model)}else{$modelSelect.val('dall-e-3')}
if(!$modelSelect.val()){$modelSelect.val('dall-e-3')}
function resolveControlsForModel(model){if(modelControls[model]){return modelControls[model]}
if(model&&model.indexOf('gpt-image-')===0&&modelControls['gpt-image']){return modelControls['gpt-image']}
return modelControls['dall-e-3']||{qualities:[{value:'hd',label:'HD (default)'},{value:'standard',label:'Standard'}],sizes:[{value:'1024x1024',label:'1024×1024 (square)'}],supports_background:!1}}
function modelSupportsTransparentBg(model){return model&&model.indexOf('gpt-image-')===0}
function updateControls(){var model=$modelSelect.val();var controls=resolveControlsForModel(model);var $qualityRow=$quality.closest('.spexo-ai-image-field-row');if(controls.qualities&&controls.qualities.length){$quality.empty();controls.qualities.forEach(function(q){$('<option/>').attr('value',q.value).text(q.label).appendTo($quality)});$qualityRow.show()}else{$quality.empty();$qualityRow.hide()}
$resolution.empty();(controls.sizes||[]).forEach(function(s){$('<option/>').attr('value',s.value).text(s.label).appendTo($resolution)})}
updateControls();$modelSelect.on('change',updateControls);var $bgCheckboxLabel=$('<label class="spexo-ai-image-bg-checkbox-label" style="margin-right:6px;margin-bottom:8px;margin-top:8px;display:none;"><input type="checkbox" class="spexo-ai-image-bg-checkbox" style="margin-right:8px;"/>Transparent background</label>');function updateBgCheckbox(){var $bgRow=$bgCheckboxLabel.closest('.spexo-ai-image-field-row');var controls=resolveControlsForModel($modelSelect.val());var showBg=controls.supports_background||modelSupportsTransparentBg($modelSelect.val());if(showBg){$bgCheckboxLabel.show();if($bgRow.length){$bgRow.show()}}else{$bgCheckboxLabel.hide().find('input').prop('checked',!1);if($bgRow.length){$bgRow.hide()}}}
$modelSelect.on('change',updateBgCheckbox);updateBgCheckbox();var $submit=$('<button type="button" class="spexo-ai-image-prompt-submit elementor-button elementor-button-primary elementor-size-sm"><img src="'+data.icon_url+'" style="width:16px;height:16px;margin-right:6px;vertical-align:middle;"/><span>Generate</span></button>').css({marginRight:'6px'});var $cancel=$('<button type="button" class="spexo-ai-image-prompt-cancel elementor-button elementor-button-link elementor-size-sm" title="Cancel"><span class="spexo-ai-cancel-icon">✕</span></button>');$promptContainer.append($input,$('<div class="spexo-ai-image-field-row"></div>').append('<label class="spexo-ai-image-field-label">Quality</label>',$quality),$('<div class="spexo-ai-image-field-row"></div>').append('<label class="spexo-ai-image-field-label">Size</label>',$resolution),$('<div class="spexo-ai-image-field-row"></div>').append('<label class="spexo-ai-image-field-label">Model</label>',$modelSelect),$('<div class="spexo-ai-image-field-row"></div>').append($bgCheckboxLabel),!is_pro?'<p class="spexo-ai-pro-notice" style="margin: 8px 0 12px 0; padding: 8px 12px; background: #fff3cd; border: 1px solid #ffc107; border-radius: 4px; font-size: 12px; color: #856404; line-height: 1.5;">⚠️ <strong>Model selection</strong> is only available in the <a href="'+(SpexoAiImageField.settings_url||'')+'" target="_blank" style="color: #0073aa; text-decoration: underline;">Pro version</a>.</p>':'',$('<div class="spexo-ai-image-field-row spexo-ai-image-field-row--buttons"></div>').append($submit,$cancel),$('<p class="spexo-ai-image-prompt-note">Complex prompts may take up to 2 minutes to process.</p>'));$outer.append($promptContainer);updateControls();updateBgCheckbox();$btn.addClass('is-active');$input.focus();$submit.on('click',function(){var prompt=$input.val().trim();if(!prompt){$input.css('border-color','red');return}
...
$btn.addClass('is-active');var selectedModel=is_pro?$modelSelect.val():'dall-e-3';var requestData={action:'spexo_ai_generate_image',nonce:SpexoAiImageField.generate_nonce,prompt:prompt,quality:$quality.val(),size:$resolution.val(),model:selectedModel};if(is_pro&&modelSupportsTransparentBg(selectedModel)&&$bgCheckboxLabel.find('input').is(':checked')){requestData.background='transparent'}
