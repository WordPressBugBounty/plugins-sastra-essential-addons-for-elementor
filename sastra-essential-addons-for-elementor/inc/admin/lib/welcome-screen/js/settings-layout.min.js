...
this.$navItems=$('.spexo-settings-nav-item');this.$panels=$('.spexo-settings-panel');this.$subLinks=$('.spexo-settings-sub-link');this.$mergedSettingsLinks=$('.tmpcoder-theme-welcome .nav-tab-wrapper .nav-tab[data-settings-panel]');this.$title=$('[data-spexo-active-title]');this.$description=$('[data-spexo-active-description]');this.$saveNotice=$('.spexo-settings-alert');this.initialSnapshot=this.$form.serialize();this.lastActiveSubpanels={};this.defaultValues={};var hashPanel=this.getActivePanelFromHash();var defaultPanel=this.$panels.filter('.is-active').data('panel')||this.$navItems.first().data('panel');this.activePanel=hashPanel||defaultPanel;this.storeDefaultValues();this.ensureDefaultSubpanels();this.restoreActiveSection();this.bindEvents();this.bindFieldChange();this.initNavHover();this.highlightInitialSubLink();this.checkDirtyState();this.checkResetSuccess()}
SpexoSettingsLayout.prototype.bindEvents=function(){var self=this;this.$navItems.on('click','.spexo-settings-nav-toggle',function(){var $item=$(this).closest('.spexo-settings-nav-item');var targetPanel=$item.data('panel');self.activatePanel(targetPanel,$item)});this.$subLinks.on('click',function(event){event.preventDefault();var $btn=$(this);var targetPanel=$btn.closest('.spexo-settings-nav-item').data('panel');var anchor=$btn.data('target');self.activatePanel(targetPanel,$btn.closest('.spexo-settings-nav-item'),anchor,!1);if(anchor){self.updateSubpanelHash(targetPanel,anchor)}});this.$mergedSettingsLinks.on('click',function(event){event.preventDefault();var $link=$(this);var hasNestedSubtabs=$link.closest('li').children('ul').children('li').length>0;var isLeafSubtab=!!$link.attr('data-settings-anchor');if(hasNestedSubtabs&&!isLeafSubtab){return}
var targetPanel=$link.attr('data-settings-panel');var anchor=$link.attr('data-settings-anchor')||null;if(!targetPanel){return}
var $navItem=self.$navItems.filter('[data-panel="'+targetPanel+'"]');self.activatePanel(targetPanel,$navItem,anchor,!1);self.syncMergedSettingsLinks(targetPanel,anchor)});$('.spexo-reset-section').on('click',function(event){event.preventDefault();self.resetCurrentSection()});this.$form.on('submit',function(){self.initialSnapshot=self.$form.serialize();self.checkDirtyState()})};SpexoSettingsLayout.prototype.bindFieldChange=function(){var self=this;this.$form.on('input change','input:not([type="button"]):not([type="submit"]):not([type="reset"]), select, textarea',function(){self.checkDirtyState()})};SpexoSettingsLayout.prototype.checkDirtyState=function(){var isDirty=this.$form.serialize()!==this.initialSnapshot;this.$form.toggleClass('spexo-settings-form--dirty',isDirty);if(this.$saveNotice&&this.$saveNotice.length){this.$saveNotice.toggleClass('is-visible',isDirty)}
...
var hashSubpanel=this.getSubpanelFromHash(panelId);var targetSubpanel=anchorId||hashSubpanel||this.lastActiveSubpanels[panelId]||this.getDefaultSubpanelId($panel);this.setActiveSubpanel(panelId,targetSubpanel,{scroll:!!anchorId||!!hashSubpanel});this.syncMergedSettingsLinks(panelId,targetSubpanel)};SpexoSettingsLayout.prototype.syncMergedSettingsLinks=function(panelId,subpanelId){if(!this.$mergedSettingsLinks.length){return}
this.$mergedSettingsLinks.closest('.nav-tab-wrapper').find('.nav-tab').removeClass('nav-tab-active');var $activeLink=$();var $activePanelLink=this.$mergedSettingsLinks.filter('[data-settings-panel="'+panelId+'"]:not([data-settings-anchor])').first();if(subpanelId){var $activeSubpanelLink=this.$mergedSettingsLinks.filter('[data-settings-panel="'+panelId+'"][data-settings-anchor="'+subpanelId+'"]').first();if($activeSubpanelLink.length){$activeLink=$activeSubpanelLink}}
if(!$activeLink.length&&$activePanelLink.length){$activeLink=$activePanelLink}
if(!$activeLink.length){$activeLink=this.$mergedSettingsLinks.first()}
if($activeLink.length){$activeLink.addClass('nav-tab-active')}
if($activeLink.length){$(document).trigger('tmpcoder:sidebar-tab-active',[$activeLink])}};SpexoSettingsLayout.prototype.updateHeader=function($item){if(!$item.length){return}
...
window.history.replaceState({},'',newUrl)}};SpexoSettingsLayout.prototype.showLoader=function(){var $loader=$('.welcome-backend-loader');if($loader.length){if(window.tmpcoderCommonLoader&&typeof window.tmpcoderCommonLoader.show==='function'){window.tmpcoderCommonLoader.show('.welcome-backend-loader')}else{$loader.fadeIn(200)}
this.$form.css('opacity','0.5');this.$form.css('pointer-events','none')}};SpexoSettingsLayout.prototype.hideLoader=function(){var $loader=$('.welcome-backend-loader');if($loader.length){if(window.tmpcoderCommonLoader&&typeof window.tmpcoderCommonLoader.hide==='function'){window.tmpcoderCommonLoader.hide('.welcome-backend-loader')}else{$loader.fadeOut(200)}
this.$form.css('opacity','1');this.$form.css('pointer-events','auto')}};SpexoSettingsLayout.prototype.showSuccessMessage=function(message){var self=this;var $alert=$('.spexo-settings-alert');if(!$alert.length){$alert=$('<div class="spexo-settings-alert" role="status" aria-live="polite"></div>');var $formContent=this.$form.find('.spexo-settings-content');if($formContent.length){$formContent.prepend($alert)}else{this.$form.prepend($alert)}}
