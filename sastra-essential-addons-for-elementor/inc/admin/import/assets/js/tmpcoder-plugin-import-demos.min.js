...
const import_process_popup_open=function(){jQuery('#import-demo-popup').fadeIn();jQuery('.tmpcoder-import-popup-wrap.tmpcoder-admin-popup-wrap').fadeIn()}
...
const should_keep_existing_demo_data=function(){return window.spexoWizardKeepExistingData===!0}
const start_import_after_reset_decision=function(){if(should_keep_existing_demo_data()){if(woocommerce_attributes_file_url){import_woocommerce_attributes_before_xml(woocommerce_attributes_file_url)}else{import_xml(xml_file_url,!1,tmpcoder_cpt_data)}
return}
reset_posts()}
function import_demo_content_start(){message_text.text(tmpcoder_ajax_object.start_import_message);import_required_files();import_process_popup_open();install_require_plugins();tmpcoder_fix_plugin_compatibility(requiredPlugins);var installPlugins=setInterval(function(){if(Object.values(requiredPlugins).every(Boolean)&&requiredTheme&&requiredFiles){start_import_after_reset_decision();clearInterval(installPlugins)}},1000)}
...
var searchTimeout=null,maingGridHtml=jQuery('.tmpcoder-import-demo-grid').html();const revealDemoGridWhenImagesReady=function(){var $page=jQuery('.tmpcoder-import-demo-page');var $images=jQuery('.main-grid .grid-item:visible .demo-preview');var pendingImages=0;if(!$page.length){return}
$page.removeClass('tmpcoder-demo-images-ready');if(!$images.length){$page.addClass('tmpcoder-demo-images-ready');return}
$images.each(function(){if(this.complete&&this.naturalWidth!==0){return}
pendingImages++;jQuery(this).one('load error',function(){pendingImages--;if(pendingImages<=0){$page.addClass('tmpcoder-demo-images-ready')}})});if(pendingImages<=0){$page.addClass('tmpcoder-demo-images-ready')}}
revealDemoGridWhenImagesReady();jQuery('.tmpcoder-import-demo-search').find('input').keyup(function(e){if(e.which===13){return!1}
...
jQuery('.main-grid .grid-item:visible').each(function(i){if(''!==tag){let title=jQuery(this).attr('data-title');if(-1===title.indexOf(tag)){jQuery('.main-grid').append(jQuery(this).remove())}}});revealDemoGridWhenImagesReady()}
