 ...
 * @since 5.6.0
 */
class TMPCODER_Choose extends Control_Choose {
	...
	public function content_template() {
		...
		<div class="elementor-control-field">
			...
			<div class="elementor-control-input-wrapper">
				<div class="elementor-choices {{ data.image_choose ? 'tmpcoder-image-choices' : 'tmpcoder-choices' }}">
					...
					<label class="elementor-choices-label elementor-control-unit-1 tooltip-target" for="<?php $this->print_control_uid( $control_uid_input_type ); ?>" data-tooltip="{{ options.title }}" title="{{ options.title }}">
						<# if( options.image ){ #>
							<img class="tmpcoder-image-option" src="{{ options.image }}" alt="{{ options.title }}" />
						<# } else if( options.text ){ #>
                            <span class="tmpcoder-text-option" alt="{{ options.title }}">{{{ options.text }}}</span>
						<# } else{ #>
